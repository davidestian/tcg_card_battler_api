...
func main() {
	...
	generalRepo := repository.NewGeneralRepository(pool)

	accountService := service.NewAccountService(accountRepo, unitRepo, inventoryRepo, teamRepo, transactor)
	inventoryService := service.NewInventoryService(inventoryRepo, unitRepo, accountRepo, transactor)
	...
	api1 := router.Group("/api/v1")
	{
		...
		route.RegisterGeneralRoutes(api1, generalHandler)
	}
